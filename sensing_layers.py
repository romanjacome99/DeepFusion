...
class ForwardCASSI(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.opt_H:
            H_init = np.random.rand(1, self.Md, self.Md, 1, self.shots)
...
class ForwardMCFA(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.opt_H:
            ...
            H_init = tf.constant_initializer(np.random.rand(1, self.input_dim[0], self.input_dim[0], self.Ld,
                                                            self.shots))
            ...
            H_init = tf.constant_initializer(
                np.random.randint(0, 2, size=(1, self.input_dim[0], self.self.input_dim[0], self.Ld,
                                              self.shots)))

            self.H = self.add_weight(name='H',
                                     shape=(1, 1, self.input_dim[0], self.input_dim[0], self.Ld, self.shots),
                                     initializer=H_init, trainable=False)
    ...
    def call(self, inputs, **kwargs):
        ...
        input_im = tf.expand_dims(tf.expand_dims(
            tf.reshape(tf.image.resize(tf.expand_dims(tf.reshape(inputs[0], [M * M, L]), -1), [M * M, Ld]),
                       [M, M, Ld]), 0), -1)
